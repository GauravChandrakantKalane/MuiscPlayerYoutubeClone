    ...
    <div className="w-full flex items-center gap-3 ">
      <div className={`w-full items-center gap-3 p-4 flex relative`}>
        ...
        <div className="flex items-start flex-col">
          <p className="textx-xl text-headingColor font-semibold">
            {`${
          ...
          >
            <RiPlayListFill className="text-textColor hover:text-headingColor cursor-pointer"></RiPlayListFill>
          </motion.i>
        </div>
        ...
        {isPlayList && <PlayListCard></PlayListCard>}
        <IoClose className="cursor-pointer" onClick={closePlayer}></IoClose>
      </div>
    </div>
  );
}
...
export const PlayListCard = () => {
  ...
  return (
    <div className="absolute left-4 bottom-24 gap-2 py-2 w-350 max-w-[350px] h-420 max-h-[420px] flex flex-col overflow-y-scroll scrollbar-thin rounded-md shadow-md bg-primary">
      {allSongs.length > 0 ? (
        allSongs.map((music, index) => {
          return (
            <motion.div
              ...
              key={index}
            >
              <IoMusicalNote className="text-textColor group-hover:text-headingColor text-2xl cursor-pointer"></IoMusicalNote>
              <div className="flex items-start flex-col">
